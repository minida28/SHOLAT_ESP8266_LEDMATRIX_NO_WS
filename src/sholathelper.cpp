...
void process_sholat()
{
  ...
  uint32_t utc_time = now;
  uint32_t local_time = localTime;

  uint8_t len;
  
  len = strlen(getDateTimeStr(utc_time));
  char buf_utc_date_time[len + 1];
  strlcpy(buf_utc_date_time, getDateTimeStr(utc_time), sizeof(buf_utc_date_time));

  len = strlen(getDateTimeStr(local_time));
  char buf_local_date_time[len + 1];
  strlcpy(buf_local_date_time, getDateTimeStr(local_time), sizeof(buf_local_date_time));
