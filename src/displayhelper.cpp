...
uint8_t get_province_index_and_max_index(const char *provinceName, uint8_t *index, uint8_t *maxIndex)
{
    PRINT("\r\n%s\r\n", __PRETTY_FUNCTION__);
...
void process_runningled_page()
{
    ...
    if (MODE == 0)
    {
        ...
        if (currentPageMode0 == 0)
        {
            ...
            char contents[sizeof(_configLocation.district)];
            if (page == 0)
            {
        ...
        else if (currentPageMode0 == 1)
        {
            ...
            if (HOUR != 0)
            {
                ...
                matrix.println(bufSECOND);

                //String "detik"
                char textSec[] = "dtk";
                // matrix.getTextBounds(bufSECONDMATRIX, 0, 0, &x1Temp, &y1Temp, &wTemp, &hTemp);
                matrix.getTextBounds(bufSECOND, 0, 0, &x1Temp, &y1Temp, &wTemp, &hTemp);
                c = c + wTemp - 3;
                matrix.setCursor(c, row0);
        ...
        else if (currentPageMode0 == 8)
        {
            ...
            char bufTimeLeftSholat[7];

            //format (add leading '0' if  less than 10) and store in the array
            sprintf_P(bufTimeLeftSholat, "%02d%02d%02d", HOUR, MINUTE, SECOND);
        ...
        else if (currentPageMode0 == 9)
        {
            ...
            if (typePage9 == 0)
            {
                ...
                //format (add leading '0' if  less than 10) and store in the array
                sprintf_P(bufNow, "%02d%02d%02d", hour12, minLocal, secLocal);
    ...
    else if (MODE == 2)
    {
        ...
        if (currentPageMode2 == 0)
        {
            ...
            // Variables for parsing regency
            static uint8_t provId = 0;
            static uint32_t startPos = 0;

            // Variables for parsing district
            static uint16_t regencyId = 0;
            static double tempLat;
            static double tempLon;

            static char bufTempProvinceName[sizeof(_configLocation.province)];
            static char bufTempTimezonestring[sizeof(_configLocation.timezonestring)];

            static char bufTempRegencyName[sizeof(_configLocation.regency)];
            static char bufTempDistrictName[sizeof(_configLocation.district)];
            ...
            static bool stateEdit_old = false;
            if (stateEdit != stateEdit_old)
            {
                stateEdit_old = stateEdit;
            ...
            if (stateEdit)
            {
                ...
                if (tempIndex != tempIndex_old)
                {
                    ...
                    else if (page == 1)
                    {
                        ...
                        PRINT("tempIndex:%d, regency name:%s\r\n\r\n",
            ...
            if (page != page_old)
            {
                ...
                if (page == 0)
                {
                    const char *initialProvinceName = _configLocation.province;

                    strlcpy(bufTempProvinceName, initialProvinceName, sizeof(bufTempProvinceName));

                    uint32_t start;

                    start = millis();
                    get_province_index_and_max_index(initialProvinceName, &index, &maxIndex);
                    tempIndex = index;
                    PRINT("Province name: %s, index: %d, maxIndex: %d\r\n", initialProvinceName, index, maxIndex);
                    PRINT("Completed in:%lums\r\n", millis() - start);
                ...
                else if (page == 1)
                {
                    ...
                    const char *provNameToSearch = bufTempProvinceName;
                    ...
                    // check if province has been changed
                    int check = strcmp(_configLocation.province, provNameToSearch);
                ...
                else if (page == 2)
                {
                    ...
                    const char *regencyNameToSearch = bufTempRegencyName;
                    ...
                    // check if regency has been changed
                    int check = strcmp(_configLocation.regency, regencyNameToSearch);
            ...
            else if (stateEdit)
            {
                ...
                else if (page == 3)
                {
                    ...
                    const char *ptr = PSTR("SIMPAN LOKASI ?");
                    strcpy_P(str, ptr);
